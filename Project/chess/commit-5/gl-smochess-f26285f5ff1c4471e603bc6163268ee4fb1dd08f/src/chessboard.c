...
void chessboard_place_block(Chessboard *cboard, Block *p, int x, int y) {
...
Block *get_block(Chessboard* c, int x, int y) {
	/* invert the position of the pieces along the y-axis */
	y = NUM_CELLS - y - 1;
...
void display_chessboard(Chessboard *cboard) {
    ...
    for (x=-0.5f; x<0.5f; x+=step){
    	...
    	for (y=-0.5f; y<0.5f; y+=step){
			...
			/* draw block at cell */
			Block *block = get_block(cboard, xcell, ycell);
			if (block) display_block(block);
    	}
   }
